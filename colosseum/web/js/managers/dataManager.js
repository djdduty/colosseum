...
function updatePaginatorDom(element) {
    ...
    const visible = element.querySelector('tbody:not(.hide)');
    if (!visible) return;
    ...
    const count = parseInt(data.count, 10); // eslint-disable-line
...
export default class DataManager {
    ...
    processAdded(element) {
        ...
        Array.from(sourceElements).forEach((sourceElement) => {
            ...
            if (!sourceElement.dataset.defer) {
    ...
    getData(element, requestedByPaginator = false) {
        ...
        */
        ...
        let extraFilters = {};
        if (source.indexOf('?') !== -1) {
            extraFilters = utils.parseUriQuery(source);
        ...
        const request = this.context.request.get(source, Object.assign({}, filters, extraFilters), { Accept });
    ...
    populateData(element, err, text, xhr) {
        ...
        */
        ...
        if (getSourcePaginator(element.id)) {
            updatePaginatorDom(element);
        }
    }

    importTemplateData(element, data) { //eslint-disable-line
